<template>
<div class="ball-box">
    <div class="interface-bg" @mousemove="getMousePosition">
        <canvas id="bgStage" :width="canvasWidth" :height="canvasHeight"> </canvas>
        <canvas id="ballStage" :width="canvasWidth" :height="canvasHeight"> </canvas>
    </div>
</div>
</template>
<script>
export default {
    ...
    methods: {
        ...
        getMousePosition(e) {
            // 全部都化成绝对值；
            if (e.layerX > this.circleX) {
                var x = e.layerX - this.circleX;
            } else {
                var x = this.circleX - e.layerX;
            }
            if (e.layerY > this.circleY) {
                var y = e.layerY - this.circleY;
            } else {
                var y = this.circleY - e.layerY;
            }
            // 现在鼠标的角度；
            this.angle = Math.atan(y / x);

            // 第一象限
            if (e.layerX > this.circleX && e.layerY < this.circleY) {
                this.angle = Math.PI - this.angle;
                // 第二象限
            } else if (e.layerX < this.circleX && e.layerY < this.circleY) {
        ...
        gameOver() {
            // 刷新当前页面；
             this.ballStage.clearRect(0, 0, this.canvasWidth, this.canvasHeight);
             this.begin();
        }
    },
    mounted() {
        this.canvasWidth = Math.floor(document.body.clientWidth);
        this.canvasHeight = Math.floor(document.body.clientHeight);
        this.circleX = Math.floor(this.canvasWidth / 2)
        this.circleY = Math.floor(this.canvasHeight / 2);
        // 选择外框
